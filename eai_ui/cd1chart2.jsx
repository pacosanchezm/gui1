    ...
    return (
      ...
      <ResponsiveContainer width={"100%"} height={"100%"} aspect={2}>
        <BarChart
          ...
          data={Data}
          onClick={v => {
            if (Activo.value === v.activePayload[0].payload.Dia) {
              setActivo({ value: null, label: "Todos" });
            } else {
              setActivo({
                value: v.activePayload[0].payload.Dia,
                label: v.activePayload[0].payload.Dia
              });
            }
          }}
        >
            ...
            {Data.map((entry, index) => {
              return (
                <Cell
                  key={`cell-${index}`}
                  stroke={"darkgreen"}
                  strokeWidth={entry.Dia === props.Activo.value ? 4 : 1}
                />
              );
            })}
  ...
  try {
    return (
      <div>
            ...
            <Transition in={props.CompStatus.Comp2()} timeout={500}>
            {state => (
              // ...
